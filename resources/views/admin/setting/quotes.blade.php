...
@section('content')
    ...
    <div class="d-flex gap-5" style="margin-left: 18rem; margin-top: 5%;">
        <div>
            ...
            <div class="my-4">
                <form action="{{ route('admin-setting.quotes-store') }}" method="POST">
                    @csrf
                    <input type="text" name="quotes" id="addQuotes" placeholder="Tambahkan Quotes"
                        class="w-100 p-3 rounded" style="background-color: #E9E9E9; border: 0.5px solid #00000050;">
                    <div class="d-flex w-50 mt-3 float-end mb-5">
                        <button type="reset" class="border-0 bg-transparent w-50"
                            style="margin-left: auto; padding: 8px;">Batal</button>
        ...
        <div class="d-flex flex-column shadow-lg"
            style="background-color: #E9E9E9; border-radius: 12px; padding: 40px; width: 40%; height: fit-content; margin-right: 32px;">
            <h6 class="text-uppercase text-center">quotes ulang tahun</h6>
            <div class="text-capitalize bg-white w-100 rounded text-center p-1 mt-5 fw-semibold ">
                {{ $quotes_ulangtahun->quote }}</div>
            <form class="mt-4" method="post"
                action="{{ route('admin-setting.quotes-ulangtahun-update', $quotes_ulangtahun->id) }}">
                @csrf
                @method('PATCH')
                <label for="quotes_ulangtahun" class="text-capitalize fw-semibold">edit quotes</label>
                <textarea name="quotes" id="quotes_ulangtahun" rows="4" placeholder="Masukkan quotes...."
                    class="bg-white w-100 border-0 rounded p-2 px-3 mt-2" required></textarea>
