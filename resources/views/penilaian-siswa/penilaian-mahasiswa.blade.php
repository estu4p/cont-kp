                ...
                <tbody>
                    @foreach ($mahasiswa as $key => $mhs)
                        <tr class="{{ $key % 2 == 0 ? 'abu' : 'putih' }}">
                            ...
                            <td>
                                @if ($mhs->status_akun == 'done')
                                    <div class="done">Done</div>
                                @elseif ($mhs->status_akun == 'aktif')
                                    <div class="aktif">Active</div>
                                @elseif ($mhs->status_akun == 'alumni')
                                    <div class="alumni">Inactive</div>
                                @endif
                            </td>
                            <td class="icon"><a href="{{ route('contributorformitra.input-nilai', $mhs->id) }}">
